...
(function() {
    ...
    (function() {
        ...
        function bindEvent(context, shape) {
            ...
            addEvent(context.canvas, isTouch ? 'touchend' : 'click', function () {
        ...
        function decorateIcon(context, icon) {
        	context.fillStyle = 'Gray';
        	context.font = '26px FontAwesome';
        	context.fillText(icon, 6, 30);
        }
    ...
    if (screen.width < 450) $('body').css('scale', '0.5');

    if (!window.parent.BizAPP.UI.Whiteboard1.readonly) {
    	...
    	addEvent(canvas, isTouch ? 'touchend' : 'mouseup', function (e) {
    		isDirty = true;
    		if (isTouch) {
    			e = e.pageX ? e : e.changedTouches.length ? e.changedTouches[0] : {
    				pageX: 0,
    				pageY: 0
    			};
    		}
    	...
    	$(document.body).append('<style>.authortool-box,.tool-box{display: none}</style>');

    if (!window.parent.BizAPP.UI.Whiteboard1._designer.initiate)
    	$(document.body).append('<style>.authortool-box{display: none}</style>');
    ...
    window.addEventListener('message', function(event) {
        ...
        else if (event.data.loadDataURL) {
        	var imageObj = new Image();
        	imageObj.onload = function () {
        		imageHandler.lastImageURL = imageObj.src;
        		imageHandler.lastImageIndex = 0;

        		imageHandler.images.push(imageObj);
        		tempContext.drawImage(imageObj, 0, 0);
        	};
        	imageObj.src = event.data.loadDataURL;
		}

        if (event.data.undo && points.length) {
						isDirty = event.data.isDirty ? true : false;
    ...
    function syncPoints(isSyncAll) {
        ...
        if (!pointsToShare.length && points.length) {
					syncData(null);
					return;
				}
