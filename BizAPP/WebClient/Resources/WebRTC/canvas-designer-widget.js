...
function CanvasDesigner() {
    ...
    function onMessage(event) {
        //
        if (!event.data) return;// || event.data.uid !== designer.uid

        if (!event.data.canvasDesignerSyncData || !!event.data.canvasDesignerSyncData) {
      			if(designer.saveCallback){
      				designer.saveCallback(event.data.canvasDesignerSyncData, event.data.isDirty);
      				return;
      			}
    ...
    designer.undo = function(index, isDirty) {
        if (!designer.iframe) return;
    ...
    designer.postMessage = function(message) {
        ...
        if(!message.uid) message.uid = designer.uid;
