...
function finder (criteria) {
  return function (node) {
    return Object.keys(criteria).every(function (key) {
      ...
      if (key === 'data') {
        var dataObj = criteria[key];
        return Object.keys(dataObj).every(function (s) { return node['data'][s] === dataObj[s]; })
      }

      if (key === 'parent') {
        var parentObj = criteria[key];
        return Object.keys(parentObj).every(function (s) { return node['parent'] && node['parent'][s] === parentObj[s]; })
      }
      ...
      if (key === 'states') {
        ...
        var states$1 = criteria[key];
        return Object.keys(states$1).every(function (s) { return node[key] && node[key][s] === states$1[s]; })
      } else {
        if (key === 'text') {
          $div.innerHTML = val;
          val = $div.innerText;
...
var TreeMixin = {
  mounted: function mounted () {
    ...
    dataProvider.then(function (data) {
      ...
      this$1.model = tree.parse(data, this$1.opts.modelParse);
...
var TreeRoot = {render: function(){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{class:{'tree': true, 'tree-loading': this.loading},attrs:{"role":"tree"}},[(_vm.filter && _vm.matches.length == 0)?[_c('div',{staticClass:"tree-filter-empty"},[_vm._v(_vm._s(_vm.opts.filter.emptyText))])]:[_c('ul',{staticClass:"tree-root"},[(_vm.opts.filter.plainList && _vm.matches.length > 0)?_vm._l((_vm.matches),function(node){return (node.visible())?_c('node',{key:node.id,attrs:{"node":node,"options":_vm.opts}}):_vm._e()}):_vm._l((_vm.model),function(node){return (node.visible())?_c('node',{key:node.id,attrs:{"node":node,"options":_vm.opts}}):_vm._e()})],2)]],2)},staticRenderFns: [],
  name: 'Tree',
  components: {
    'node': TreeNode
  },
