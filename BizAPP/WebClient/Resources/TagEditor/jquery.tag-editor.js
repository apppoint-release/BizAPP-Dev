...
(function($){
    ...
    $.fn.tagEditor = function(options, val, blur){
        ...
        if (typeof options == 'string') {
            ...
            selector.each(function(){
                ...
                if (options == 'getTags')
                    ...
                    // insert new tag
                    $('<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag"></div>'+get_deletetag()+'</div></li>').appendTo(ed).find('.tag-editor-tag')
        ...
        function get_deletetag() {
            var delStr = "";
            if (!o.readOnly)
                delStr = '<div class="tag-editor-delete"><i></i>';
            return delStr;
        }
        ...
        return selector.each(function(){
            var el = $(this), tag_list = [], tag_id_list = [];
            ...
            // markup for new tag
            var new_tag = '<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag"></div>'+get_deletetag()+'</div></li>';            
            ...
            // helper: update global data
            function update_globals(init, delId){
                var old_tags = tag_list.toString();
                tag_list = $('.tag-editor-tag:not(.deleted)', ed).map(function (i, e) {
                    var val = $.trim($(this).hasClass('active') ? $(this).find('input').val() : $(e).text());
                    if (val) return val;
                }).get();
                tag_id_list = $('.tag-editor-tag:not(.deleted)', ed).map(function (i, e) {
                    var val = $.trim($(e).attr('bza-id'));
                    if (val) return val;
                }).get();
                ed.data('tags', tag_list);
                el.val(tag_id_list.join(o.delimiter[0]));
                // change callback except for plugin init
                if (!init) if (old_tags != tag_list.toString()) o.onChange(el, ed, tag_list, tag_id_list, delId);
                set_placeholder();
            }
            ...
            ed.on('click', '.tag-editor-delete', function(e){
                ...
                update_globals(false, tag.attr('bza-id'));
                return false;
            });
            ...
            ed.on('click', '.tag-editor-tag', function (e) {
                ...
                if ((!$(this).hasClass('active')) && !o.readOnly && $(e.target).attr('bza-id')==null) {
                    var tag = $(this).html();
            ...
            function split_cleanup(input) {
                ...
                for (i = 0; i < sub_tags.length; i++) {
                    ...
                    if (tag) {
                        ...
                        old_tags.push(tag);
                        li.before('<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag">'+tag+'</div>'+get_deletetag()+'</div></li>');
                    }
                }
            ...
            ed.on('keydown', 'input', function(e){
                ...
                if (e.which == 8) {
                    if ($t.val()) { }
                    else $t.closest('li').prev('li').find('.tag-editor-delete').click();
                    e.stopPropagation();
                }
                    // left/up key + backspace key on empty field
                else if ((e.which == 37 || !o.autocomplete && e.which == 38) && !$t.caret() || e.which == 8 && !$t.val()) {
            ...
            if (o.readOnly) {
                ...
                for (i = 0; i < tags.length; i++) {
                    ...
                    if (tag) {
                        if (o.forceLowercase) tag = tag.toLowerCase();
                        tag_list.push(tag);
                        ed.append('<li><div class="tag-editor-spacer">&nbsp;' + o.delimiter[0] + '</div><div class="tag-editor-tag">' + tag + '</div>' + get_deletetag() + '</div></li>');
                    }
                }
            }
            else {
                var tags = JSON.parse(el.attr('bza-tag'));
                for (var key in tags) {
                    var tag = $.trim(tags[key].replace(/ +/, ' '));
                    if (tag) {
                        if (o.forceLowercase) tag = tag.toLowerCase();
                        tag_list.push(tag);
                        ed.append('<li><div class="tag-editor-spacer">&nbsp;' + o.delimiter[0] + '</div><div class="tag-editor-tag" bza-id="' + key + '">' + tag + '</div>' + get_deletetag() + '</div></li>');
                    }
                }
            }
